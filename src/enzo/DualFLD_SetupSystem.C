...
/***********************************************************************
/
/  Dual Flux-Limited Diffusion Solver, linear system setup routine 
/
/  written by: Daniel Reynolds
/  date:       November 2014
/
/  PURPOSE: Fills HYPRE matrix and right-hand side objects corresponding
/           to the FLD radiation linear system of equations.
...
int DualFLD::SetupSystem(HierarchyEntry *ThisGrid, int XrUv, float *E,
  ...
  if (E==NULL)
    ENZO_FAIL("DualFLD::SetupSystem ERROR: no E array!");
  if (Opacity==NULL)
    ENZO_FAIL("DualFLD::SetupSystem ERROR: no Opacity array!");
  if (Eta==NULL)
  ...
  if (XrUv) {
    rUn    = UVUnits;
    rUn0   = UVUnits0;
    xLbdry = UVBdryType[0][0];
    xRbdry = UVBdryType[0][1];
    yLbdry = UVBdryType[1][0];
    yRbdry = UVBdryType[1][1];
    zLbdry = UVBdryType[2][0];
    zRbdry = UVBdryType[2][1];
  } else {
    rUn    = XrUnits;
    rUn0   = XrUnits0;
    xLbdry = XrBdryType[0][0];
    xRbdry = XrBdryType[0][1];
    yLbdry = XrBdryType[1][0];
    yRbdry = XrBdryType[1][1];
    zLbdry = XrBdryType[2][0];
    zRbdry = XrBdryType[2][1];
  }
  ...
  for (int i2=0; i2<LocDims[2]; i2++) {
	...
	for (int idx=0; idx<6; idx++)
	  if (isinf(vals[idx]) || isnan(vals[idx])) {
	    ...
	    ENZO_FAIL("NaN error in DualFLD::SetupSystem\n");
	  }
	...
	if ((rank > 2) && (OnBdry[2][0]) && (i2==0)) {
	  if (zLbdry == 1)         // Dirichlet
	    vals[s_zl] = 0.0;
	  else if (zLbdry == 2) {  // Neumann
	    vals[s_c] += vals[s_zl];
	    vals[s_zl] = 0.0;
	  }
	}
	...
	if ((rank > 1) && (OnBdry[1][0]) && (i1==0)) {
	  if (yLbdry == 1)         // Dirichlet
	    vals[s_yl] = 0.0;
	  else if (yLbdry == 2) {  // Neumann
	    vals[s_c] += vals[s_yl];
	    vals[s_yl] = 0.0;
	  }
	}
	...
	//    x-left face
	if ((OnBdry[0][0]) && (i0==0)) {
	  if (xLbdry == 1)         // Dirichlet
	    vals[s_xl] = 0.0;
	  else if (xLbdry == 2) {  // Neumann
	    vals[s_c] += vals[s_xl];
	    vals[s_xl] = 0.0;
	  }
	}
