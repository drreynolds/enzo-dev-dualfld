			 ...
			 float *Opacity, float *Eta, float &rhsnorm)
{
  ...
  float dxi  = 1.0/dx[0]/lUn;
  float dyi  = 1.0/dx[1]/lUn;
  float dzi  = 1.0/dx[2]/lUn;
  float dxi0 = 1.0/dx[0]/lUn0;
  float dyi0 = 1.0/dx[1]/lUn0;
  float dzi0 = 1.0/dx[2]/lUn0;
...
#pragma omp for reduction(+:rhsnorm) schedule(static) default(shared)
  for (int i2=0; i2<n3[2]; i2++) {
	...
	vals[s_c] = 1.0 + dtfac*(afac + c_light*kap)     // self
                      + dxfac*(D[s_xl]+D[s_xr])
	              + dyfac*(D[s_yl]+D[s_yr])
	              + dzfac*(D[s_zl]+D[s_zr]);
