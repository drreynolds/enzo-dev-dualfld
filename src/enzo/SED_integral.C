...
/           convertHz -- flag denoting whether the result of the 
/                        integral (performed over hnu in eV) should 
/                        be converted to Hz.
/
************************************************************************/
#include <stdlib.h>
...
float SED_integral(SED &sed, float a, float b, bool convertHz) {
  ...
  // return 1.0 if the sed frequency is inside the interval, otherwise 0.0
...
// composite numerical integration of a specified integrand over 
// a specified (definite) interval
float SED_composite_integral(SED &sed, float a, float b, int n) {
