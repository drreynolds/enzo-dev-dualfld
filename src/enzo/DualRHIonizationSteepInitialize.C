/***********************************************************************
/
/  INITIALIZE DUAL FLD TEST -- IONIZATION TEST IN A R^{-2} DENSITY PROFILE
/
				    ...
				    TopGridData &MetaData, int local)
{
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR)
    printf("Entering DualRHIonizationSteepInitialize routine\n");
  ...
  float NumDensity           = 3.2;           // [cm^{-3}]
  float DensityRadius        = 1.14375e-1;    // [code units]
  float DensityCenter0       = 0.0;
  float DensityCenter1       = 0.0;
  float DensityCenter2       = 0.0;
  float Temperature          = 100.0;         // [K]
  ...
  if (MetaData.RadHydroParameterFname != NULL) {
	...
	ret += sscanf(line, "DualFLD_NumDensity = %"FSYM, &NumDensity);
	ret += sscanf(line, "DualFLD_DensityRadius = %"FSYM, &DensityRadius);
  ...
  while (Temp != NULL) {
    if (Temp->GridData->DualRHIonizationSteepInitializeGrid(
                        NumDensity, DensityRadius, DensityCenter0, 
			DensityCenter1, DensityCenter2, X0Velocity, 
			X1Velocity, X2Velocity, IEnergy, 
			XrRadiation, UVRadiation, InitialFractionHII, 
			InitialFractionHeII, InitialFractionHeIII, local) == FAIL) {
      fprintf(stderr, "Error in DualRHIonizationSteepInitializeGrid.\n");
      return FAIL;
    }
