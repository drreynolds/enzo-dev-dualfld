/***********************************************************************
/
/  DUAL FLD INITIALIZE RADIATION-HYDRODYNAMICS TEST -- CONSTANT FIELD INITIAL CONDITIONS
/
...
int DualRadConstTestInitialize(FILE *fptr, FILE *Outfptr, 
  ...
  if (MyProcessorNumber == ROOT_PROCESSOR)
    printf("Entering DualRadConstTestInitialize routine\n");
  ...
  while (Temp != NULL) {
    if (Temp->GridData->DualRadConstTestInitializeGrid(
      ...
      fprintf(stderr, "Error in DualRadConstTestInitializeGrid.\n");
      return FAIL;
    }
