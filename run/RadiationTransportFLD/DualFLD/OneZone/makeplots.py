...
def _Xray(field, data):
    return (data["Xray_Radiation"])
def _convertlogXray(data):
    return ( data.convert("MassUnits")/data.convert("TimeUnits") / 
             data.convert("TimeUnits")/data.convert("cm"))
add_field("Xray", take_log=True, function=_Xray, 
          convert_function=_convertlogXray, 
          display_name="Xray\; Radiation\; Energy\; Density", 
...
for it in range(te+1):
    sdump = repr(it).zfill(4)
    pfile = 'DD' + sdump + '/data' + sdump
    pf = load(pfile)
    Nresults["time"].append(pf.current_time*pf["TimeUnits"])
    Nresults["Xray"].append(na.average(pf.h.grids[0]["Xray"]))
    Nresults["xHI"].append(na.average(pf.h.grids[0]["xHI"]))
    Nresults["x"].append(na.average(pf.h.grids[0]["xHII"]))
    Nresults["T"].append(na.average(pf.h.grids[0]["Temperature"]))
    Nresults["E"].append(na.average(pf.h.grids[0]["TotalEnergy"]))
    Nresults["HIkph"].append(na.average(pf.h.grids[0]["HI_kph"]))
    Nresults["photogamma"].append(na.average(pf.h.grids[0]["PhotoGamma"]))
